...
class RegistrarPago:
    ...
    def execute(self, alumno_id: str, monto: float, concepto: str) -> Pago:
        # Generar un ID simple para el ejemplo
        pago_id = f"pago-{datetime.now().strftime('%Y%m%d%H%M%S%f')}"
        nuevo_pago = Pago(id=pago_id, alumno_id=alumno_id, monto=monto, fecha=datetime.now(), concepto=concepto)
