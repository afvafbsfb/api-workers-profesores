"""
Script sencillo para probar endpoints GET del openapi.yml actual
...
class TestResult:
    ...
    def line(self) -> str:
        ...
        mark = "INFO" if self.is_info else ("PASS" if self.ok else "FAIL")
...
def build_headers() -> Dict[str, str]:
    headers = {
        "User-Agent": "simple-get-endpoint-tester/0.3",
        "Accept": "application/json",
    }
    if API_KEY:
        # El backend espera X-Api-Key (si se usa la protección)
...
def safe_get_json(url: str) -> Dict[str, Any]:
    ...
    return {"status": resp.status_code, "json": data, "error": err, "text": resp.text}


def test_get(path: str, expected_status: List[int], dynamic: bool = False) -> TestResult:
    method = "GET"
    real_path = _ensure_leading_slash(path)
...
def collect_endpoints() -> List[Tuple[str, str, List[int]]]:
    """Construye la lista de endpoints a probar, con detalle de empresa si hay IDs."""
    base_endpoints: List[Tuple[str, str, List[int]]] = [
        ...
        ("/vlodeiro/secretaria/alumnos", "Listado de alumnos", [200]),
        ("/vlodeiro/secretaria/alumnos/buscar?nombre=Alumno", "Buscar alumnos por nombre (genérico)", [200, 400]),
    ]

    empresa_ids = discover_empresa_ids(MAX_EMPRESA_IDS)
    if empresa_ids:
        for eid in empresa_ids:
            base_endpoints.append((f"/vlodeiro/empresa/empresa/{eid}", f"Detalle de empresa {eid}", [200]))
    else:
        if FALLBACK_PLACEHOLDER:
            base_endpoints.append(("/vlodeiro/empresa/empresa/1", "Detalle empresa placeholder (sin IDs)", [200, 404]))
        else:
...
def summarize(results: List[TestResult], duration: float) -> int:
    ...
    passed = sum(1 for r in real_tests if r.ok)
    failed = total - passed

    print("\nResumen:")
    print(f"  Base URL: {BASE_URL}")
    print("  API Key: (usada)" if API_KEY else "  API Key: (no proporcionada)")
